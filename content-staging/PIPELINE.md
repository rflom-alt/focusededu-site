...
For each feed item: slug = last path segment of its `url`. The item is NEW only if (a) no post with that slug exists in src/lib/posts-data.json AND (b) no content-staging/pending-<id>.json exists for it AND (c) its id is not in content-staging/skipped-ids.json.
ID TYPE TRAP — the feed's `id` is a STRING ("2948023"), not a number. Coerce with String() on BOTH sides of every id comparison; a bare `skipSet.has(item.id)` or `stagedSet.has(item.id)` against numbers fails silently and re-stages an item you already killed. This bit once on 2026-08-11. After computing the new-item list, assert that every skip-list id still present in the feed was actually excluded, and log it — a skip list that matches nothing is a bug, not a clean run. For each NEW item, apply this exact cleaning checklist to content_html:
